...
export function GameBar({stats=[],onPause,accent=C.accent,label}){
  return(
    <div style={{width:"100%",display:"flex",alignItems:"center",gap:12,flexWrap:"wrap",justifyContent:"space-between"}}>
      <div style={{display:"flex",gap:16,alignItems:"center",flexWrap:"wrap"}}>
        {label&&<div style={{color:accent,fontSize:11,fontWeight:700,letterSpacing:1.4}}>{label}</div>}
        {stats.map(s=>(
          <div key={s.label} style={{textAlign:"center",minWidth:38}}>
            <div style={{color:C.faint,fontSize:9,letterSpacing:1.2,fontWeight:600}}>{s.label}</div>
            <div style={{color:s.color||C.text,fontWeight:700,fontSize:17,fontVariantNumeric:"tabular-nums"}}>
              {s.value}{s.sub&&<span style={{color:C.faint,fontSize:11,fontWeight:600}}>{s.sub}</span>}
            </div>
          </div>
        ))}
      </div>
      <button onClick={onPause} aria-label="Pause"
        ...
        style={{display:"flex",alignItems:"center",gap:8,background:C.surface,border:`1px solid ${C.border}`,color:C.muted,
          borderRadius:10,padding:"0 16px",height:44,fontSize:13,fontWeight:600,cursor:"pointer",
